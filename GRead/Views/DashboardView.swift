...
struct DashboardView: View {
    ...
    private func loadAchievements(userId: Int) async {
        do {
            ...
            await MainActor.run {
                achievements = response.achievements.sorted { $0.unlockedAt ?? "" > $1.unlockedAt ?? "" }
            }
        } catch {
    ...
    var body: some View {
        VStack(spacing: 8) {
            Image(systemName: achievement.isUnlocked ? "trophy.fill" : "lock.fill")
                .font(.largeTitle)
                .foregroundColor(achievement.isUnlocked ? themeColors.warning : themeColors.textSecondary)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: 12)
                .stroke(achievement.isUnlocked ? themeColors.warning.opacity(0.5) : themeColors.border, lineWidth: 1)
        )
    }
}
